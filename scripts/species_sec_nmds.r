...
species_sec_long <- merged_data |>
  ...
  summarise(
    n_plots_present = sum(presence),
    no_plots       = max(no_plots),
    .groups = "drop"
  ) |>
  mutate(frac = round(n_plots_present / no_plots, 2)) |>
  filter(frac != 0) |> 
  distinct(year, section, veg_type, taxon_code, n_plots_present, no_plots,frac)

species_sec_wide <-
  species_sec_long |>
  select(-n_plots_present, -no_plots) |> 
  tidyr::pivot_wider(
...
community_matrix <- species_sec_wide |> 
...
plot_metadata <- merged_data |>
  ...
  select(sec_year_vt, year, veg_type) |> 
  distinct()
...
nmds_plot_data <- nmds_scores |>
  left_join(plot_metadata, by = "sec_year_vt") |>
...
metadata <- species_sec_long %>%
  mutate(sec_year_vt = paste(section, year, veg_type, sep = "_")) %>%
  filter(sec_year_vt %in% rownames(community_matrix)) %>%
  arrange(match(sec_year_vt, rownames(community_matrix))) |> 
...
all(rownames(community_matrix) == metadata$sec_year_vt)  # should be TRUE
...
stopifnot(all(metadata_agg$sec_year_vt == rownames(community_matrix)))
...
pairwise_adonis <- function(dist_matrix, factors, permutations = 999) {
  ...
  do.call(rbind, results)  # place outside both loops
}
...
veg_types <- unique(species_sec_long$veg_type)
...
run_nmds_for_veg <- function(vt) {
  ...
  plot_metadata_vt <- species_sec_long %>%
    filter(veg_type == vt) %>%
    mutate(sec_year_vt = paste(section, year, veg_type, sep = "_")) %>%
    distinct(sec_year_vt, year, veg_type)
