...
disp_df$sec_year_vt <- metadata_agg$sec_year_vt

disp_df <- disp_df %>%
  left_join(metadata_agg %>% select(sec_year_vt, section, veg_type), by = "sub_year_vt")
...
lmm_func_sec_factor <- lmer(distance ~ factor(year) + (1 | section), data = disp_df)
summary(lmm_func_sec_factor)
...
veg_types <- unique(pft_sec_long$veg_type)
